...
import { 
  ...
  ShieldCheck, Zap, BarChart3, Settings, X, Copy, Layout, PenTool
} from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { analyzeLog } from './services/gemini';

function App() {
  const [activeTab, setActiveTab] = useState('analytics'); // 'analytics' ou 'jira'
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col items-center p-6 font-sans">
      <div className="relative z-10 w-full max-w-6xl">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 border-b border-white/5 pb-8">
          ...
          <nav className="flex bg-slate-900/50 p-1 rounded-2xl border border-white/5">
            <button 
              ...
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'analytics' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
            >
              <Layout size={14} /> AI ANALYTICS
            </button>
            <button 
              onClick={() => setActiveTab('jira')}
              className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'jira' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
            >
              <PenTool size={14} /> JIRA COMPOSER
            </button>
            <button onClick={() => setShowSettings(true)} className="p-2.5 text-slate-500 hover:text-white transition-colors ml-2 border-l border-slate-800">
              <Settings size={18} />
            </button>
          </nav>
        </header>
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[32px] shadow-3xl overflow-hidden min-h-[500px] flex flex-col">
                    <div className="bg-slate-50 px-8 py-5 border-b flex justify-between items-center">
                      ...
                      <button 
                        ...
                        className="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-lg text-xs font-bold hover:bg-blue-50 hover:text-blue-600 transition-all"
                      >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[600px]">
              <div className="flex flex-col bg-slate-900/40 border border-white/5 rounded-[32px] p-6 shadow-2xl">
                <div className="flex justify-between items-center mb-4 px-2">
                  <h3 className="text-white font-bold text-xs uppercase tracking-widest flex items-center gap-2"><PenTool size={14} /> Validação Técnica</h3>
                ...
                <textarea 
                  ...
                  placeholder="Descreva a falha técnica e insira os códigos aqui..."
                  className="flex-grow bg-slate-950/50 border border-white/5 rounded-2xl p-6 text-sm font-mono text-blue-300 outline-none focus:border-blue-500/50 transition-all resize-none shadow-inner"
                />
              </div>
              ...
              <div className="flex flex-col bg-white rounded-[32px] overflow-hidden shadow-3xl">
                <div className="bg-slate-50 px-8 py-5 border-b flex justify-between items-center">
                  ...
                  <button 
                    onClick={() => copyToClipboard(jiraInput, "resposta JIRA")}
                    className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-lg"
                  >
        ...
        <footer className="mt-16 text-center text-[10px] font-black uppercase tracking-[0.5em] text-slate-700">
          Analista Lead: Anderson Leoni • APOIO B2B CVCCORP
        </footer>
      </div>

      {/* Modal Vault (Engrenagem) */}
      <AnimatePresence>
        {showSettings && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6">
            <div className="bg-slate-900 border border-white/10 p-10 rounded-[40px] max-w-md w-full shadow-3xl">
              ...
              <input 
                ...
                className="w-full bg-slate-800 border border-slate-700 p-4 rounded-xl mb-6 text-white text-sm outline-none focus:border-blue-500 transition-all"
              />
